export default {
    add_word_left: "Add word before",
    add_word_right: "Add word after",
    add_segment_before: "Add segment before",
    add_segment_after: "Add segment after",
    merge_segment_up: "Merge into previous segment",
    split_segment_after: "Split segment after this word",
    delete_segment: "Delete segment",
    segment_actions: "Segment actions",
    segment_start: "Start",
    segment_end: "End",
    auto_scroll: "Auto-scroll",
    jump_to_playback: "Jump to playback",
    cancel: "Cancel",
    unsaved_changes: "Unsaved changes",
    discard: "Discard",
    duration: "Duration",
    file: "File",
    language: "Language",
    source: "Source",
    checksum: "Checksum",
    loading_transcript: "Loading transcript…",
    no_changes: "No changes",
    saving: "Saving…",
    dismiss: "Dismiss",
    transcript_saved: "Transcript saved",
    save_failed: "Saving the transcript failed: {error}",
    rename_transcript: "Rename transcript",
    transcript_label: "Transcript label",
    open_panel: "Open panel",
    close_panel: "Close panel",
    settings: "Settings",
    named_entities: "Named entities",
    entity_per: "Person",
    entity_loc: "Location",
    entity_org: "Organization",
    entity_date: "Date",
    entity_type: "Type",
    mention_confidence: "Mention confidence",
    word_section: "Word",
    mention_section: "Mention",
    segment_section: "Segment",
    processing: "Processing upload…",
    remove_word: "Remove word",
    remove_mention: "Remove mention",
    set_as_mention: "Set as mention",
    no_mention: "Not a named entity",
    extend_mention_left: "Extend to the left",
    extend_mention_right: "Extend to the right",
    reduce_mention_left: "Trim from the left",
    reduce_mention_right: "Trim from the right",
    redaction_section: "Redaction",
    no_redaction: "Not redacted",
    set_as_redaction: "Redact this word",
    remove_redaction: "Remove redaction",
    extend_redaction_left: "Extend to the left",
    extend_redaction_right: "Extend to the right",
    reduce_redaction_left: "Shorten from the left",
    reduce_redaction_right: "Shorten from the right",
    redaction_reason: "Reason",
    save_transcript: "Save",
    confidence: "Confidence",
    timestamps: "Timestamps",
    show_confidence: "Show confidence scores",
    show_edits: "Show edits",
    show_entities: "Show entities",
    shortcuts: "Shortcuts",
    shortcut_play_pause: "Play / pause",
    shortcut_back: "Back 5 s",
    shortcut_forward: "Forward 5 s",
    shortcut_volume: "Volume",
    shortcut_mute: "Mute / unmute",
    shortcut_speed: "Playback speed",
    shortcut_fullscreen: "Fullscreen (video)",
    shortcut_jump: "Jump to playback",
    add: "Add",
    save: "Save",
    add_speaker: "Add speaker",
    edit_speaker: "Rename speaker",
    delete_speaker: "Delete speaker",
    delete_speaker_confirm: ": delete?",
    speaker_name: "Speaker name",
    speaker_color: "Speaker color",
    speakers: "Speakers",
    no_speaker: "No speaker",
    view: "View",
    upload: "Upload",
    word_actions: "Word actions",
    media_player: {
        play: "Play",
        pause: "Pause",
        seek_back: "Back 5 s",
        seek_forward: "Forward 5 s",
        playback_speed: "Playback speed",
        mute: "Mute",
        unmute: "Unmute",
        volume: "Volume",
        fullscreen: "Fullscreen",
        seek_to_position: "Jump to position",
    },
    queue: {
        cancel_hint:
            "To stop an upload, use the cancel button — you can resume incomplete uploads later.",
        tab_switch_hint:
            "You can switch to other tabs and keep working — just don't close or leave this page until the upload finishes.",
        large_files_hint:
            "Large files may take a while to upload depending on your connection speed.",
        pending: "pending",
        uploading: "uploading",
        uploading_progress: "Uploading file {current} of {total}",
        uploaded: "uploaded",
        cancelled: "cancelled",
        incomplete: "incomplete",
        checksum_complete: "Checksum verified",
        transferred: "Transferred",
        total: "Total",
        speed: "Speed",
        eta: "ETA",
        checksum_status_pending: "pending",
        checksum_status_generating: "generating",
        checksum_status_transferring: "transferring",
        checksum_status_complete: "complete",
    },
};
